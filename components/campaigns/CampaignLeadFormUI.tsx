...
//   - ad attribution and all raw qualification values
...
function buildPayload(config: CampaignConfig, values: Values): CampaignEnquiryPayload {
  ...
  const qualificationFields = config.form.steps.flatMap((s) => s.fields)
  const summary = qualificationFields
    .map((f) => {
...
export default function CampaignLeadFormUI({ config }: { config: CampaignConfig }) {
  const { form } = config
  const qualSteps = form.steps
  const contactIndex = qualSteps.length
  const totalSteps = qualSteps.length + 1
  ...
  const isContact = step === contactIndex
  ...
  function handleNext() {
    const next = validateFields(qualSteps[step].fields)
    setErrors(next)
    ...
    if (!formStartedTracked.current) {
      ...
      trackEvent('campaign_form_started', { campaign_id: config.id, campaign_source: 'paid-campaign' })
    }
    const target = step + 1
    setStep(target)
    focusHeading(target === contactIndex ? 'campaign-contact-heading' : `campaign-step-${target}-heading`)
  }
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const next = validateFields(form.contact.fields, () => {
  ...
  if (status === 'success') {
    return (
      <div className="rounded-2xl border border-gray-200 bg-white p-7 text-center shadow-xl shadow-black/10 sm:p-8">
        ...
        <p className="mt-2 text-sm leading-relaxed text-gray-600">
          Thanks — your details are with our project team. We reply within one business day. If it’s urgent,
          call us on{' '}
  ...
  return (
    <form
      ...
      className="rounded-2xl border border-gray-200 bg-white p-5 shadow-xl shadow-black/20 sm:p-6 lg:p-7"
    >
      ...
      <div className="mb-5">
        <p className="text-base font-black text-[#0d1b2a] sm:text-lg">{config.hero.formHeading}</p>
        <div className="mt-3 flex items-center gap-3">
          <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-gray-100" aria-hidden="true">
            <div className="h-full rounded-full bg-[#3e91ce] transition-all duration-300" style={{ width: `${progress}%` }} />
          </div>
          <span className="shrink-0 text-xs font-bold uppercase tracking-wide text-[#30505b]">
      ...
      {/* Qualification steps */}
      {qualSteps.map((s, i) => (
        <div key={s.heading} hidden={step !== i} className="space-y-5">
          <div>
            <h3 id={`campaign-step-${i}-heading`} tabIndex={-1} className="text-base font-bold text-[#0d1b2a] focus:outline-none">
              {s.heading}
            </h3>
            {s.intro && <p className="mt-1 text-sm text-gray-500">{s.intro}</p>}
          </div>
          <fieldset className="space-y-5">
            <legend className="sr-only">{s.heading}</legend>
            <FieldGrid fields={s.fields} values={values} errors={errors} setValue={setValue} toggleCard={toggleCard} />
          </fieldset>
          <StepNav
            onBack={i > 0 ? handleBack : undefined}
            onNext={handleNext}
            nextLabel={i === contactIndex - 1 ? 'Continue to contact details' : 'Continue'}
          />
        </div>
      ))}

      {/* Contact step */}
      <div hidden={!isContact} className="space-y-5">
        <div>
          <h3 id="campaign-contact-heading" tabIndex={-1} className="text-base font-bold text-[#0d1b2a] focus:outline-none">
            {form.contact.heading}
          </h3>
          {form.contact.intro && <p className="mt-1 text-sm text-gray-500">{form.contact.intro}</p>}
        </div>

        <fieldset className="space-y-5">
          <legend className="sr-only">{form.contact.heading}</legend>
          <FieldGrid fields={form.contact.fields} values={values} errors={errors} setValue={setValue} toggleCard={toggleCard} />
        </fieldset>
        ...
        <div className="flex flex-col gap-3 sm:flex-row-reverse">
          <button
            ...
            className="inline-flex min-h-[52px] flex-1 items-center justify-center gap-2 rounded-lg bg-[#0d1b2a] px-6 text-sm font-semibold tracking-wide text-white transition-colors hover:bg-[#30505b] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3e91ce] focus-visible:ring-offset-2 disabled:opacity-60"
          >
...
// ── Step navigation (qualification steps) ─────────────────────────────────────
function StepNav({
  onBack,
  onNext,
  nextLabel,
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row-reverse">
      <button
        type="button"
        onClick={onNext}
        className="inline-flex min-h-[52px] flex-1 items-center justify-center gap-2 rounded-lg bg-[#0d1b2a] px-6 text-sm font-semibold tracking-wide text-white transition-colors hover:bg-[#30505b] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3e91ce] focus-visible:ring-offset-2"
      >
        {nextLabel}
        <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} aria-hidden="true">
        ...
        <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
          {f.options?.map((opt, i) => {
            ...
            return (
              <label
                key={opt}
                className={`flex min-h-[46px] cursor-pointer items-center gap-2.5 rounded-lg border px-3.5 py-2.5 text-sm transition-colors ${
        ...
        <div className="flex flex-wrap gap-2.5">
          {f.options?.map((opt, i) => {
            ...
            return (
              <label
                key={opt}
                className={`flex min-h-[44px] cursor-pointer items-center gap-2 rounded-lg border px-4 py-2 text-sm transition-colors ${
                  checked ? 'border-[#0d1b2a] bg-[#0d1b2a]/[0.04] text-[#0d1b2a]' : 'border-gray-300 text-gray-600 hover:border-[#3e91ce]'
                }`}
              >
                <input
