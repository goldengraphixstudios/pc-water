...
export default function PostEditorForm({ post }: { post?: CmsPost | null }) {
  ...
  const initialForm = useRef(JSON.stringify(toFormState(post)))
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      // Update the reference so "saved" becomes true
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_290px]">
          ...
          <div className="space-y-4">
            <Section title="Publishing">
              ...
              <Field label="Read Time" hint="Leave blank to auto-calculate">
                <input
                  value={form.readTime ?? ''}
                  onChange={(e) => update('readTime', e.target.value)}
                  className="field"
                  placeholder={bodyWords > 0 ? `~${Math.ceil(bodyWords / 200)} min read` : '5 min read'}
                />
              </Field>
            </Section>
