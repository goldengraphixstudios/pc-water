...
export default function ProjectEditorForm({ project, availableClassifications = [], availableTags = [] }: Props) {
  ...
  const initialForm = useRef(JSON.stringify(toFormState(project)))
  const formRef = useRef<HTMLFormElement>(null)

  useEffect(() => {
    setSaved(JSON.stringify(form) === initialForm.current)
  }, [form])
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      initialForm.current = JSON.stringify(payload)
      setSaved(true)
      setSaveSuccess(true)
      setTimeout(() => setSaveSuccess(false), 3000)
      if (!project) router.push('/cms/projects')
    })
  }
  ...
  const bodyWords     = wordCount(form.content)
  const seoDescLen    = (form.seoDescription ?? '').length
  const seoDescOver   = seoDescLen > SEO_DESC_MAX
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-2">
              ...
              {project && !saved && (
                <span className="text-[11px] text-amber-500 dark:text-amber-400 font-medium flex-shrink-0">● Unsaved</span>
              )}
              {project && saved && saveSuccess && (
                <span className="text-[11px] text-emerald-500 dark:text-emerald-400 font-medium flex-shrink-0">✓ Saved</span>
              )}
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          <span className="hidden lg:flex items-center gap-1 text-[11px] text-slate-400 dark:text-slate-600">
            <kbd className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-[#1E2235] text-slate-500 dark:text-slate-400 font-mono text-[10px]">⌘S</kbd>
            to save
          </span>
          {project && (
            <button
              ...
              disabled={isPending}
              className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-lg border border-red-200 dark:border-red-900/50 text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/40 text-[13px] font-medium transition-colors disabled:opacity-40"
            >
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_320px]">
          ...
          <div className="space-y-4">
            ...
            <Section title="Project Details">
              ...
              <div className="block">
                <div className="flex items-baseline justify-between mb-1.5">
                  <span className="text-xs font-semibold text-slate-600 dark:text-slate-400">Body</span>
                  <span className="text-[11px] text-slate-400 dark:text-slate-600">
                    {bodyWords.toLocaleString()} word{bodyWords !== 1 ? 's' : ''}
                    {bodyWords > 0 && (
                      <span className="ml-1 text-slate-300 dark:text-slate-700">
                        · ~{Math.ceil(bodyWords / 200)} min read
                      </span>
                    )}
                  </span>
                </div>
            ...
            <Section title="SEO">
              ...
              <div className="block">
                <div className="flex items-baseline justify-between mb-1.5">
                  <span className="text-xs font-semibold text-slate-600 dark:text-slate-400">SEO Description</span>
                  <span className={`text-[11px] font-medium ${seoDescOver ? 'text-red-500' : seoDescLen > 140 ? 'text-amber-500' : 'text-slate-400 dark:text-slate-600'}`}>
                    {seoDescLen}/{SEO_DESC_MAX}
                  </span>
                </div>
                <span className="block text-xs text-slate-400 dark:text-slate-600 mb-1.5 -mt-1">Recommended: 160 characters or fewer</span>
                <textarea
                  value={form.seoDescription ?? ''}
                  onChange={(e) => update('seoDescription', e.target.value)}
                  className={`field min-h-20 resize-none ${seoDescOver ? 'border-red-300 dark:border-red-800 focus:border-red-400' : ''}`}
