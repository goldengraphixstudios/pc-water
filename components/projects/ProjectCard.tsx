...
function StandardCard({ project }: { project: ProjectItem }) {
  return (
    ...
    >
      <div className="relative h-[72px] w-[72px] flex-shrink-0 overflow-hidden rounded bg-[#162538] sm:h-40 sm:w-full sm:rounded-none sm:rounded-t-lg">
        {project.imageSrc ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img
            src={project.imageSrc}
      ...
      <div className="flex min-w-0 flex-1 flex-col sm:p-4">
        ...
        <h3 className="mb-1.5 line-clamp-2 text-sm font-bold leading-snug text-[#30505b] transition-colors group-hover:text-[#2a72ad] sm:mb-2 sm:text-[15px]">
          {project.title}
        </h3>
